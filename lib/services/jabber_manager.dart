...
class JabberManager implements DataChangeEvents {
  ...
  static bool mainTimerTaskRunning = false;
  ...
  Future<void> startMainTimer() async {
    int defaultCheckRegisterTimer = 20;
    int checkRegisterTimer = 1;
    Timer.periodic(const Duration(seconds: 1), (Timer t) async {
      if (mainTimerTaskRunning) {
        Log.d(tag, 'previous task still running');
        return;
      }
      if (registered) {
        connectedTime += 1;
      ...
      mainTimerTaskRunning = true;
      checkRegisterTimer -= 1;
      if (checkRegisterTimer <= 0) {
        checkRegisterTimer = defaultCheckRegisterTimer;
        Log.d(
            tag,
            'XMPP: is registered: $registered'
            ', stopFlag $stopFlag'
            ...
            for (PendingMessageModel pendingMessage in pendingMessages) {
              if ((pendingMessage.time ?? 0) < (now - longInterval)) {
                ...
                if (chatMessage.id != null) {
                  ...
                  if (chatMessage.customText != null && chatMessage.customText != '') {
                    Map<String, dynamic> customText = jsonDecode(chatMessage.customText!);
                    if (customText['path'] != null && customText['path'] != '') {
                      ...
                      if (await file.exists()) {
                        ...
                        String? putUrl =
                        await JabberManager.flutterXmpp?.requestSlot(
                            fname, filesize);

                        if (putUrl != null && putUrl != '') {
                          Map<String, dynamic> response = await requestPutFile(
                              putUrl, file);
                  ...
                  if (chatMessage.msgtype == 'groupchat') {
                    await sendGroupMessage(chatMessage.from ?? '',
                        ...
                        pk: chatMessage.mid ?? '', now: chatMessage.time ?? 0,
                        customText: chatMessage.customText ?? '', isResend: true);
                  } else {
  ...
  void checkReceivedMessage(MessageChat messageChat,
    ...
    */
    ...
    if (messageChat.type == 'Ack') {
       int deleted = await PendingMessageModel().deleteByUid(messageChat.id ?? '');
    ...
    UserSettingsModel().getUser().then((userSettings) async {
      if (userSettings != null) {
        ...
        await userSettings.updatePartial(
            userSettings.id, {'isXmppRegistered': isRegistered ? 1 : 0});
      }
      await getRoster();
      if (isRegistered) {
        initData();
  ...
  Future<dynamic> addMyRoster(String phone) async {
  ...
  Future<ChatMessageModel> sendMessage(String from, String to, String body,
      {String pk = '', int now = 0, String customText = '', bool isResend = false}) async {
  ...
  Future<void> sendMessage2Db(ChatMessageModel msg, {bool isResend = false}) async {
    if (isResend) {
